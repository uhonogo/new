...
// comments list
function custom_comment_form($arg,$class='btn-variant',$id='submit'){
    ob_start();
    comment_form($arg);
    $form = ob_get_clean();
...
function custom_list_comments($comment, $args, $depth){
    $path = get_template_directory() . '/template-parts/list_commetnts.php';
...
// option list
  ...
  $wrap_before = '<div class="breadcrumbs pt-3 pb-3" itemscope itemtype="http://schema.org/BreadcrumbList">';
...
function my_search_form( $form ) {
	$form = '<form role="search" method="get" id="searchform" class="search" action="' . home_url( '/' ) . '" >
				<label hidden for="s">' . __( 'Search for:' ) . '</label>
